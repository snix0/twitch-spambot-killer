...
class SpamKiller():
    ...
    commands = {
        ...
        '^!banfile': 'ban_from_file',
        '^!wlf': 'whitelist_from_file',
        }

    mitigation_active = 0

    # Load settings from files
    with open("blacklist_date.txt") as f:
        lines = f.readlines()
        for line in lines:
            d = line.strip()
    ...
    with open("whitelist.txt") as f:
        ...
        for line in lines:
            usr = line.strip()
            whitelist_users.append(usr)

    # self.chat(self.s, 'Whitelisting users: {}'.format(ban_users))
    print("Whitelisting users: {}".format(whitelist_users))
    for usr in whitelist_users:
        whitelisted_users.append(usr)
            ...
            if self.mitigation_active:
                for chatter in self.chatter_list:
                    ...
                    try:
                        ...
                        print("Viewer list:",chatter, user_date)

                        if user_date in self.banned_dates:
                            self.punish(chatter)
                    except:
                        continue
    ...
    def process_chat(self, chat_object):
        ...
        if self.mitigation_active == 1 and user not in self.banned_users and user not in self.whitelisted_users:
            ...
            user_date = ""
            try:
                user_date = get_creation_date(user)
                print("Got creation date: ", user, user_date)

                if user_date in self.banned_dates:
                    print("Banning {}".format(user))
                    self.punish(user)
            except:
            ...
            if re.search(command, msg):
                ...
                # Manual unlisting of a date, can also !stopbans to clear the list 
                    ...
                    if input:
                        ...
                        if epoch in self.banned_dates:
                            self.banned_dates.remove(epoch)
                            print('Removing date {} from blacklist'.format(epoch))
                        else:
                            print('Date not found in blacklisted dates'.format(epoch))
                        print("Banned dates changed to:", self.banned_dates)
                    else:
                ...
                # Automatic lookup of a user's creation date blacklisting it.
                    ...
                    try:
                        if target_user not in self.whitelisted_users:
                            self.whitelisted_users.remove(target_user)
                        else:
                            print(target_user, "not in whitelisted users:", self.whitelisted_users)
                    except:
                        pass
                    return
                # Stops bans and clears blacklist
                    ...
                    # self.banned_dates = []
                    # self.chat(self.s, 'Turning off bot mitigation.')
                    print("Turning off bot mitigation.")
                    return

                # Starts banning users
                elif action == 'start_banning' and user in self.admin_list:
                    self.mitigation_active = 1
                    # self.chat(self.s, 'Turning on bot mitigation ═══█❚')
                    print("Turning on bot mitigation.")
                    return
                ...
                # Unbans and prevents future banning of the user
                    ...
                    try:
                        ...
                        return
                    if target_user == 'all':
                        print("Whitelisting with chatter_list: ",self.chatter_list)
                        self.chat(self.s, 'Whitelisting all users in current chat: {}'.format(",".join(self.chatter_list)))
                        for target_user in self.chatter_list:
                            self.whitelist_user(target_user, True)
                        return
                    else:
                        self.whitelist_user(target_user)
                    return
                elif action == 'whitelist_from_file' and user in self.admin_list:
                    whitelist_users = []
                    with open("whitelist.txt") as f:
                        lines = f.readlines()
                        for line in lines:
    ...
    def punish(self, chatter):
        if self.punishment == 'ban':
            # if chatter in self.checked_users:
                # self.checked_users.remove(chatter)
...
def get_creation_date(user):
    ...
    while 1:
        ...
        try:
            r = requests.get('https://api.twitch.tv/kraken/users?login={}'.format(user), headers = headers)
        except:
            time.sleep(1)
            continue
